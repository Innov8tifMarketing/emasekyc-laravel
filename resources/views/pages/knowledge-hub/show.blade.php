    ...
    <article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            @foreach($post->tags as $tag)
                <a href="{{ route('resources.knowledge-hub.index', ['tag' => $tag->slug]) }}" class="badge badge-soft">{{ $tag->name }}</a>
            @endforeach
        </div>
        ...
        <div class="mt-12 pt-8 border-t border-base-300">
            <a href="{{ route('resources.knowledge-hub.index') }}" class="btn btn-outline btn-sm">&larr; Back to Knowledge Hub</a>
        </div>
    </article>
</x-layout>
